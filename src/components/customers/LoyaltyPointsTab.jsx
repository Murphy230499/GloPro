...
export default function LoyaltyPointsTab({ onChanged }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const combined = [
        ...services.map(s => ({ id: s.id, name: s.name, type: t('customers.points.service', 'DỊCH VỤ') })),
        ...products.map(p => ({ id: p.id, name: p.name, type: t('customers.points.product', 'SẢN PHẨM') }))
      ];
  ...
  if (loading) {
    return <div className="py-12 text-center text-xs text-slate-400">{t('customers.points.loading_config', 'Đang tải cấu hình tích điểm...')}</div>;
  }
    ...
    <div className="grid md:grid-cols-12 gap-5 items-start">
      ...
      <div className="col-span-12 bg-amber-50/50 rounded-2xl p-4 border border-amber-100 flex flex-wrap items-center justify-between gap-3">
        <div className="text-left space-y-0.5">
          <div className="font-bold text-xs text-slate-800 flex items-center gap-1.5"><ShieldAlert className="w-4 h-4 text-amber-500" /> {t('customers.points.simulate_title', 'Cài đặt & Mô phỏng Reset điểm tích luỹ')}</div>
          <div className="text-[10px] text-slate-400 font-semibold">{t('customers.points.simulate_desc', 'Bấm nút để mô phỏng dọn dẹp điểm tích lũy của những khách hàng quá chu kỳ không ghé spa.')}</div>
        </div>
        <button
        ...
        >
          <RefreshCw className={`w-3.5 h-3.5 ${runningReset ? 'animate-spin' : ''}`} /> {t('customers.points.simulate_btn', 'Mô phỏng reset điểm')}
        </button>
      </div>
      ...
      <div className="col-span-12 md:col-span-6 bg-white rounded-3xl p-5 border border-slate-100 shadow-sm space-y-5">
        <div className="flex justify-between items-center border-b border-slate-100 pb-2">
          <h3 className="font-bold text-sm text-slate-800">{t('customers.points.rules_title', 'Quy tắc tích luỹ điểm')}</h3>
          <button onClick={handleSave} disabled={saving} className="px-3.5 py-1.5 rounded-xl bg-amber-500 text-white text-xs font-bold hover:bg-amber-600 transition-colors flex items-center gap-1.5 shadow-sm">
            <Save className="w-3.5 h-3.5" /> {saving ? t('common.saving', 'Đang lưu...') : t('customers.points.save_btn', 'Lưu cấu hình')}
          </button>
        </div>
        ...
        <div className="space-y-4 text-left">
          ...
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex items-center justify-between gap-3">
            <div className="space-y-0.5">
              <span className="block text-xs font-bold text-slate-700">{t('customers.points.exchange_rate', 'Tỷ lệ quy đổi điểm')}</span>
              <span className="text-[10px] text-slate-400 font-semibold">{t('customers.points.exchange_rate_desc', 'Quy đổi số tiền chi tiêu của hóa đơn sang 1 điểm tích lũy.')}</span>
            </div>
          ...
          <div className="space-y-2">
            <span className="block text-xs text-slate-400 px-1">{t('customers.points.earn_on_pay', 'Tích điểm khi thanh toán')}</span>
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="flex items-center gap-2 p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 cursor-pointer select-none">
                ...
                <span className="text-xs font-medium text-slate-600">{t('customers.points.use_service', 'Dùng dịch vụ')}</span>
              </label>
              ...
              <label className="flex items-center gap-2 p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 cursor-pointer select-none">
                ...
                <span className="text-xs font-medium text-slate-600">{t('customers.points.buy_product', 'Mua sản phẩm')}</span>
              </label>
              ...
              <label className="flex items-center gap-2 p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 cursor-pointer select-none">
                ...
                <span className="text-xs font-medium text-slate-600">{t('customers.points.buy_package', 'Mua gói dịch vụ')}</span>
              </label>
              ...
              <label className="flex items-center gap-2 p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 cursor-pointer select-none">
                ...
                <span className="text-xs font-medium text-slate-600">{t('customers.points.buy_course', 'Mua liệu trình')}</span>
              </label>
              ...
              <label className="flex items-center gap-2 p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 cursor-pointer select-none">
                ...
                <span className="text-xs font-medium text-slate-600">{t('customers.points.buy_cash_card', 'Mua thẻ tiền mặt')}</span>
              </label>
            </div>
          </div>
          ...
          <div className="space-y-2 pt-1">
            <span className="block text-xs text-slate-400 px-1">{t('customers.points.reward_points', 'Điểm thưởng hoạt động')}</span>
            ...
            <div className="grid grid-cols-2 gap-3.5">
              <div>
                <label className="flex items-center gap-2 mb-1.5 cursor-pointer select-none">
                  ...
                  <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide">{t('customers.points.book_advance', 'ĐẶT LỊCH TRƯỚC')}</span>
                </label>
                <input type="number" disabled={!earnOnBooking} value={bookingPoints} onChange={(e) => setBookingPoints(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-amber-500 disabled:opacity-40 text-slate-700 bg-white" placeholder={t("customers.points.reward_placeholder", "Số điểm thưởng...")} />
              </div>
              ...
              <div>
                <label className="flex items-center gap-2 mb-1.5 cursor-pointer select-none">
                  ...
                  <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide">{t('customers.points.referral', 'GIỚI THIỆU KHÁCH')}</span>
                </label>
                <input type="number" disabled={!earnOnReferral} value={referralPoints} onChange={(e) => setReferralPoints(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-amber-500 disabled:opacity-40 text-slate-700 bg-white" placeholder={t("customers.points.reward_placeholder", "Số điểm thưởng...")} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-2 border-t border-slate-100">
            <span className="block text-xs text-slate-400 px-1">{t('customers.points.reset_schedule', 'Lịch reset điểm tích luỹ')}</span>
            
            <div className="grid grid-cols-2 gap-3.5">
              <div>
                <label className="block text-xs text-slate-400 mb-1">{t('customers.points.reset_cycle', 'Chu kỳ reset điểm')}</label>
                <div className="relative">
                  <select value={resetSchedule} onChange={(e) => setResetSchedule(e.target.value)} className="w-full pl-3 pr-10 py-2.5 rounded-xl border border-slate-200 text-sm outline-none bg-white text-slate-700 appearance-none focus:border-amber-500">
                    <option value="none">{t('customers.points.no_reset', 'Không reset')}</option>
                    <option value="annually">{t('customers.tiers.period_year', 'Hằng năm')}</option>
                    <option value="half_yearly">{t('customers.tiers.period_6m', 'Hằng 6 tháng')}</option>
                    <option value="quarterly">{t('customers.tiers.period_quarter', 'Hằng quý')}</option>
                    <option value="inactivity_duration">{t('customers.points.inactive_duration', 'Xét khoảng thời gian không ghé')}</option>
                  </select>
              ...
              {resetSchedule === 'inactivity_duration' && (
                <div>
                  <label className="block text-xs text-slate-400 mb-1">{t('customers.points.max_inactive_days', 'Số ngày không ghé tối đa')}</label>
      ...
      <div className="col-span-12 md:col-span-6 bg-white rounded-3xl p-5 border border-slate-100 shadow-sm flex flex-col min-h-[460px]">
        <div className="border-b border-slate-100 pb-2">
          <h3 className="font-bold text-sm text-slate-800 text-left">{t('customers.points.exclude_list', 'Danh sách loại trừ tích điểm')}</h3>
          <p className="text-[10px] text-slate-400 font-semibold text-left">{t('customers.points.exclude_desc', 'Tích chọn các dịch vụ hoặc sản phẩm cụ thể muốn LOẠI TRỪ không cho tích điểm.')}</p>
        </div>
        ...
        <div className="flex items-center gap-2 border border-slate-200 rounded-xl px-2.5 py-1.5 mt-3 shrink-0">
          ...
          <input value={searchCatalog} onChange={(e) => setSearchCatalog(e.target.value)} placeholder={t("customers.points.exclude_search", "Tìm dịch vụ/sản phẩm loại trừ...")} className="text-xs bg-transparent outline-none flex-grow" />
        </div>

        {/* Items List scroll */}
        <div className="overflow-y-auto flex-grow max-h-[300px] space-y-2 mt-4 pr-1">
          {filteredCatalog.length === 0 ? (
            <div className="py-12 text-center text-xs text-slate-400">{t('customers.points.exclude_empty', 'Không tìm thấy sản phẩm/dịch vụ nào')}</div>
          ) : (
