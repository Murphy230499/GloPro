    ...
    <div className="space-y-5">
      ...
      <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100/50 flex flex-wrap items-center justify-between gap-3 text-left">
        <div className="text-left space-y-0.5">
          <div className="font-bold text-xs text-slate-800 flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-amber-500" /> {t('customers.tiers.auto_review', 'Xét duyệt nâng & hạ hạng tự động')}</div>
          <div className="text-[10px] text-slate-400 font-semibold">{t('customers.tiers.auto_review_desc', 'Chạy quét toàn bộ chi tiêu và điểm tích luỹ khách hàng theo chu kỳ thiết lập.')}</div>
        </div>
        <button
        ...
        >
          <Play className="w-3.5 h-3.5 fill-white" /> {runningCheck ? t('customers.tiers.running_review', 'Đang xét hạng...') : t('customers.tiers.run_review', 'Chạy xét duy trì hạng')}
        </button>
      </div>
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm space-y-4">
        <div className="flex justify-between items-center border-b border-slate-100 pb-2">
          <h3 className="font-bold text-sm text-slate-800">{t('customers.tiers.list_title', 'Danh sách hạng thành viên hiện tại')}</h3>
        </div>

        {loading ? (
          <div className="py-12 text-center text-xs text-slate-400">{t('customers.tiers.loading', 'Đang tải cấu hình hạng...')}</div>
        ) : tiers.length === 0 ? (
          <div className="py-12 text-center text-xs text-slate-400 font-sans">{t('customers.tiers.empty', 'Chưa có hạng thành viên nào được định nghĩa')}</div>
        ) : (
          <div className="space-y-3">
            {tiers.map((tier) => (
              <div key={tier.id} className="p-4 rounded-2xl border border-slate-100 bg-slate-50 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <span className="w-6 h-6 rounded-full shrink-0 flex items-center justify-center text-white shadow-2xs" style={{ background: tier.color }}>
                    {getTierIcon(tier.name, tier.id, "w-3 h-3 text-white")}
                  </span>
                  <div className="text-left font-sans">
                    <div className="font-bold text-xs text-slate-700">{tier.name}</div>
                    <div className="text-[10px] text-slate-400 mt-0.5 font-normal">
                      {t('customers.tiers.condition_spend', 'Điều kiện: Chi tiêu')} <span className="font-semibold text-slate-600">{formatVND(tier.min_spend)}</span> {t('customers.tiers.or', 'hoặc')} <span className="font-semibold text-slate-600">{tier.min_points} {t('customers.points', 'điểm')}</span>
                    </div>
                    <div className="text-[10px] text-slate-400 mt-0.5 font-normal">
                      {t('customers.tiers.benefit_discount', 'Quyền lợi: Giảm')} {tier.discount_percent > 0 ? `${tier.discount_percent}%` : formatVND(tier.discount_amount)} {t('customers.tiers.invoice_word', 'hóa đơn')} • {t('customers.tiers.cycle', 'Chu kỳ:')} {tier.maintenance_period === 'upgrade_duration' ? `${tier.maintenance_days} ${t('customers.tiers.days', 'ngày')}` : getPeriodLabels(t)[tier.maintenance_period]}
                    </div>
                  </div>
                </div>

                <div className="flex items-center gap-1">
                  <button onClick={() => handleEdit(tier)} className="p-2 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-100 transition-colors" title="Chỉnh sửa"><Edit3 className="w-4 h-4" /></button>
                  <button onClick={() => handleDelete(tier.id)} className="p-2 text-red-400 hover:text-red-600 rounded-lg hover:bg-red-50 transition-colors" title="Xóa"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            ))}
          </div>
