    ...
    <div className="space-y-4 font-sans text-left">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="p-4 rounded-2xl bg-white border border-slate-100 shadow-2xs">
          <div className="text-xs text-slate-400 font-medium">Tổng Đợt Điều Chuyển Kho</div>
          <div className="text-xl font-bold text-slate-800 mt-1">{stockTransfers.length} phiếu</div>
        </div>

        <div className="p-4 rounded-2xl bg-white border border-slate-100 shadow-2xs">
          <div className="text-xs text-slate-400 font-medium">Đang Vận Chuyển</div>
          <div className="text-xl font-bold text-amber-600 mt-1">{pendingCount} phiếu</div>
        </div>

        <div className="p-4 rounded-2xl bg-white border border-slate-100 shadow-2xs">
          <div className="text-xs text-slate-400 font-medium">Đã Nhận Hàng Hoàn Tất</div>
          <div className="text-xl font-bold text-emerald-600 mt-1">{completedCount} phiếu</div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 bg-white p-3 rounded-2xl border border-slate-100 shadow-2xs">
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input 
            ...
            placeholder="Tìm mã phiếu chuyển, chi nhánh xuất, chi nhánh nhận..."
          ...
          <table className="w-full text-left text-xs font-sans">
            <thead className="bg-slate-50 border-b border-slate-200 font-semibold text-slate-600">
              <tr>
                ...
                <th className="py-3.5 px-4">Kho xuất (Chi nhánh đi)</th>
                <th className="py-3.5 px-4">Kho nhận (Chi nhánh đến)</th>
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-slate-400 font-medium">Không tìm thấy phiếu điều chuyển kho nào</td>
                </tr>
              ) : (
                filtered.map((t) => {
                  ...
                  return (
                    <React.Fragment key={t.id}>
                      <tr className="hover:bg-indigo-50/20 transition-colors">
                        ...
                        <td className="py-3.5 px-4 text-slate-500 font-normal">{t.date}</td>
                        <td className="py-3.5 px-4 font-medium text-slate-800">{t.from_branch_name}</td>
                        <td className="py-3.5 px-4 font-bold text-indigo-900">{t.to_branch_name}</td>
                        ...
                        <td className="py-3.5 px-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            {isPending && (
                              <button
                                onClick={() => onConfirmReceiveTransfer(t.id)}
                        ...
                        <tr className="bg-slate-50/80">
                          <td colSpan={6} className="p-4">
                            <div className="bg-white rounded-xl p-3 border border-slate-200/80 space-y-2">
                              <div className="text-xs font-bold text-slate-800">Chi tiết sản phẩm điều chuyển ({t.code}):</div>
                              <div className="overflow-x-auto">
                                <table className="w-full text-left text-[11px]">
                                  <thead className="bg-slate-100 font-bold text-slate-600 border-b border-slate-200">
                                    <tr>
                                      ...
                                      <th className="p-2 text-center">Số lượng điều chuyển</th>
                                    </tr>
                                  </thead>
