...
export default function StockOutModal({ products, onClose, onSave }) {
  ...
  const handleRemoveItem = (index) => {
    if (items.length <= 1) return toast.error(t('inventory.err_at_least_one_stock_out', 'Phiếu xuất phải có ít nhất 1 sản phẩm'));
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (items.length === 0) return toast.error(t('inventory.err_select_stock_out_products', 'Vui lòng chọn sản phẩm xuất kho'));
      ...
      >
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base text-slate-800">{t('inventory.title_create_stock_out', 'Tạo Phiếu Xuất Kho')}</h3>
              <p className="text-xs text-slate-400 font-medium">{t('inventory.stock_out_subtitle', 'Trừ tự động số lượng tồn kho sản phẩm')}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.reason_stock_out_label', 'Lý do xuất kho')} <span className="text-red-500">*</span></label>
            <select 
              value={reason}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-bold text-slate-800">{t('inventory.list_export_products', 'Danh sách sản phẩm xuất kho')}</label>
              <button
                type="button"
              ...
              <table className="w-full text-left text-xs font-sans">
                <thead className="bg-slate-50 border-b border-slate-200 font-bold text-slate-600">
                  <tr>
                    ...
                    <th className="py-2.5 px-3 w-24 text-center">{t('inventory.col_stock_out_qty', 'Số lượng xuất')}</th>
                    <th className="py-2.5 px-3 w-36 text-right">{t('inventory.cost_price_vnd', 'Giá vốn (VNĐ)')}</th>
                    <th className="py-2.5 px-3 w-36 text-right">{t('inventory.total_cost_out', 'Tổng vốn xuất')}</th>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.note_detail_label', 'Ghi chú chi tiết')}</label>
            <textarea 
              value={note}
              onChange={(e) => setNote(e.target.value)}
              placeholder={t('inventory.ph_note_stock_out', 'Nhập lý do chi tiết hoặc số chứng từ xuất liên quan...')}
              rows={2}
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 flex justify-between items-center text-xs">
            <span className="text-slate-600 font-bold">{t('inventory.total_stock_out_cost_label', 'Tổng giá trị vốn xuất kho:')}</span>
          ...
          <div className="flex gap-2.5 pt-2">
            ...
            >
              {t('inventory.btn_confirm_stock_out', 'Xác Nhận Xuất Kho')}
            </button>
          </div>
        </form>
      </div>
