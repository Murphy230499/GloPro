...
export default function SupplierModal({ supplier, onClose, onSave }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.name.trim()) return toast.error(t('inventory.err_enter_supplier_name', 'Vui lòng nhập Tên Nhà cung cấp'));
    onSave(form);
  };
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base text-slate-800">
                {supplier?.id ? t('inventory.title_edit_supplier', 'Chỉnh sửa Nhà cung cấp') : t('inventory.title_add_supplier', 'Thêm mới Nhà cung cấp')}
              </h3>
              <p className="text-xs text-slate-400 font-medium">{t('inventory.supplier_subtitle', 'Quản lý hồ sơ đối tác và công nợ mua hàng')}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.supplier_code_label', 'Mã NCC')}</label>
              <input 
                type="text" 
                value={form.code}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.tax_code_label', 'Mã số thuế')}</label>
              <input 
                type="text" 
                value={form.tax_code}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.supplier_name_label', 'Tên Nhà cung cấp')} <span className="text-red-500">*</span></label>
            <input 
              type="text" 
              value={form.name}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.phone_label', 'Số điện thoại')}</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.email_contact_label', 'Email liên hệ')}</label>
              <div className="relative">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.contact_person_label', 'Người đại diện liên hệ')}</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.current_debt_vnd_label', 'Công nợ hiện tại (VNĐ)')}</label>
              <div className="relative">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.warehouse_address_label', 'Địa chỉ kho / trụ sở')}</label>
            <div className="relative">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">{t('inventory.note_label', 'Ghi chú')}</label>
            <textarea 
              value={form.note}
          ...
          <div className="flex gap-2.5 pt-3 border-t border-slate-100">
            ...
            >
              {t('inventory.btn_save_supplier', 'Lưu Nhà Cung Cấp')}
            </button>
          </div>
        </form>
      </div>
