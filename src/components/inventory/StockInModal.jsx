...
export default function StockInModal({ suppliers, products, onClose, onSave }) {
  ...
  const handleRemoveItem = (index) => {
    if (items.length <= 1) return toast.error('Phiếu nhập phải có ít nhất 1 sản phẩm');
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (items.length === 0) return toast.error('Vui lòng chọn sản phẩm nhập kho');
      ...
      >
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base text-slate-800">Tạo Phiếu Nhập Kho</h3>
              <p className="text-xs text-slate-400 font-medium">Cộng dồn số lượng tồn kho & tự động tính giá vốn</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Chọn Nhà Cung Cấp <span className="text-red-500">*</span></label>
              <select 
                value={supplierId}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Lý do nhập kho</label>
              <input 
                type="text" 
                value={reason}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-bold text-slate-800">Danh sách sản phẩm nhập kho</label>
              <button
                type="button"
              ...
              <table className="w-full text-left text-xs font-sans">
                <thead className="bg-slate-50 border-b border-slate-200 font-bold text-slate-600">
                  <tr>
                    ...
                    <th className="py-2.5 px-3 w-36 text-right">Đơn giá nhập (VNĐ)</th>
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-2">
            ...
            <div className="flex justify-between items-center gap-3">
              <span className="text-xs text-slate-600 font-bold">Thanh toán cho Nhà cung cấp (VNĐ):</span>
              <input 
                type="number"
                value={paidAmount}
            ...
            <div className="flex justify-between items-center text-xs pt-1 border-t border-slate-200">
              <span className="text-slate-500 font-medium">Ghi nợ vào công nợ NCC:</span>
