    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" onClick={onClose}>
      ...
      <div className="relative bg-white w-full md:max-w-md rounded-3xl p-6 shadow-2xl relative text-left flex flex-col max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <input value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} placeholder="Tên dịch vụ" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-xs outline-none focus:border-purple-400 text-slate-700 bg-white" />

          <div>
            <label className="block font-bold text-slate-500 mb-1 text-[11px]">Nhóm dịch vụ</label>
            <select value={f.group_id} onChange={(e) => setF({ ...f, group_id: e.target.value })} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-xs outline-none focus:border-purple-400 text-slate-700 bg-white">
          ...
          <textarea value={f.description} onChange={(e) => setF({ ...f, description: e.target.value })} placeholder="Mô tả" rows={2} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-xs outline-none focus:border-purple-400 text-slate-700 bg-white" />
          ...
          <div>
            ...
            <div className="flex gap-1.5 mt-1">
              <select value={accProduct} onChange={(e) => setAccProduct(e.target.value)} className="flex-1 px-2.5 py-2.5 rounded-xl border border-slate-200 text-xs outline-none focus:border-purple-400 text-slate-700 bg-white">
        ...
        <div className="flex gap-2 pt-4 border-t border-slate-150/50 mt-4 shrink-0">
          ...
          <button onClick={() => (f.name ? onSave(f) : toast.error('Nhập tên dịch vụ'))} className="flex-1 py-2.5 rounded-xl bg-purple-500 hover:bg-purple-600 text-white font-bold text-xs shadow-sm transition-all font-sans">Lưu</button>
        </div>
      </div>
    </div>
  );
