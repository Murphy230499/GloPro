    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" onClick={onClose}>
      ...
      <div className="relative bg-white w-full md:max-w-md rounded-3xl p-6 shadow-2xl relative text-left flex flex-col max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <input value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} placeholder="Tên liệu trình" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-xs outline-none focus:border-purple-500 text-slate-700 bg-white" />
          ...
          <div>
            <label className="block font-medium text-slate-500 mb-1 text-[11px]">Nhóm liệu trình</label>
            <div className="relative">
              ...
              >
                <option value="">— Chọn nhóm liệu trình —</option>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label className="block font-medium text-slate-500 mb-1 text-[11px]">Giá bán (VNĐ)</label>
            ...
            <div>
              <label className="block font-medium text-slate-500 mb-1 text-[11px]">Hạn dùng (tháng)</label>
            ...
            <div>
              <label className="block font-medium text-slate-500 mb-1 text-[11px]">Hạn dùng (ngày)</label>
          ...
          <div>
            <label className="block font-medium text-slate-500 mb-1 text-[11px]">Dịch vụ trong liệu trình</label>
            ...
            {f.services.length > 0 && (
              <div className="mt-2 space-y-2">
                ...
                <div className="flex justify-between text-xs text-slate-500 px-1">
                  ...
                  <span>Giá bán LT: <b className="text-purple-500">{formatVND(f.price)}</b></span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex gap-2 pt-4 border-t border-slate-150/50 mt-4 shrink-0">
          ...
          <button onClick={() => {
            if (!f.name) return toast.error('Nhập tên liệu trình');
            const payload = { ...f };
            delete payload.group;
            onSave(payload);
          }} className="flex-1 py-2.5 rounded-xl bg-purple-500 text-white font-bold text-xs shadow-sm hover:opacity-95 transition-all font-sans">Lưu</button>
        </div>
      </div>
    </div>
  );
