...
export default function RunPayrollModal({ isOpen, onClose, staffData, dateRange, onRunPayroll }) {
  ...
  // Reset selected staff to ALL whenever modal opens
  useEffect(() => {
    if (isOpen) {
      setSelectedStaffIds(staffData.map(s => s.id));
    }
  }, [isOpen, staffData]);
  ...
  // Calculate totals for selected staff only
  const selectedStaffData = staffData.filter(s => selectedStaffIds.includes(s.id));
  
  const totals = selectedStaffData.reduce((acc, row) => {
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left min-w-max border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {staffData.map(row => {
                    const isSelected = selectedStaffIds.includes(row.id);
                    return (
                      <tr key={row.id} className={`${isSelected ? 'bg-white' : 'bg-slate-50/50 opacity-50'} hover:bg-slate-50 transition-colors`}>
                        <td className={`py-4 px-5 sticky left-0 z-10 ${isSelected ? 'bg-white' : 'bg-slate-50/50'} border-r border-slate-100`}>
                          <div className="flex items-center gap-4">
                            <button 
                            onClick={() => toggleStaff(row.id)}
                            className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full transition-colors duration-200 ease-in-out focus:outline-none ${isSelected ? 'bg-orange-500' : 'bg-slate-200'}`}
                          >
                            <span aria-hidden="true" className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${isSelected ? 'translate-x-2' : '-translate-x-2'}`} />
                          </button>
                          <div className="flex items-center gap-3">
                            <Avatar src={row.avatar_url} name={row.name} size={28} color={row.avatar_color} />
                            <span className="text-sm font-normal text-slate-800 whitespace-nowrap">{row.name}</span>
                          </div>
                        </div>
                        </td>
                        ...
                        <td className={`py-4 px-5 text-sm font-normal text-slate-800 text-right sticky right-0 z-10 border-l border-slate-100 shadow-[-4px_0_10px_rgba(0,0,0,0.02)] ${isSelected ? 'bg-white' : 'bg-slate-50/50'}`}>
