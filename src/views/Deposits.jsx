...
export default function DepositsView() {
  ...
  const handleDelete = async (e, id) => {
    e.stopPropagation();
    if (!window.confirm('Bạn có chắc chắn muốn xoá phiếu cọc này?')) return;
    try {
      await base44.entities.Deposit.delete(id);
      setDeposits(deposits.filter(d => d.id !== id));
    } catch (err) {
      console.error(err);
      alert('Không thể xoá phiếu cọc');
    }
  };
    ...
    <div className="flex-1 overflow-auto bg-slate-50/50">
      <div className="p-8 max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Quản lý Đặt cọc</h1>
            <p className="text-slate-500 mt-1">Quản lý tiền cọc của khách hàng</p>
          </div>
          <button 
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden flex-1 flex flex-col">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div className="relative max-w-sm w-full">
              ...
              <input
                type="text"
                placeholder="Tìm kiếm mã cọc, khách hàng..."
          ...
          <div className="flex-1 overflow-auto">
            {loading ? (
              ...
              <div className="p-16 text-center">
                ...
                <h3 className="text-lg font-bold text-slate-800 mb-1">Chưa có đặt cọc nào</h3>
                <p className="text-slate-500 text-sm">Hãy tạo đặt cọc đầu tiên cho khách hàng</p>
              </div>
            ) : (
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 text-slate-500 font-medium sticky top-0 z-10 border-b border-slate-200 shadow-sm">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right">Số tiền yêu cầu</th>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          ...
                          {d.status === 'paid' ? 'Đã thu' : d.status === 'pending' ? 'Chờ thu' : d.status}
                        </span>
                      </td>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm print:bg-white print:p-0">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl flex flex-col font-body animate-in fade-in zoom-in-95 duration-200 print:shadow-none print:w-full">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4 no-print px-6 pt-5">
              <h2 className="text-lg font-bold text-slate-800">In phiếu cọc</h2>
            ...
            <div className="printable-receipt bg-slate-50 rounded-2xl border border-dashed border-slate-200 p-6 mx-6 mb-2 mt-0 font-mono text-xs text-slate-800 space-y-5 shadow-inner">
              <div className="text-center space-y-2 pb-4 border-b border-slate-200 border-dashed">
                ...
                <div className="text-[10px] text-slate-400">Ngày in: {new Date().toLocaleDateString('vi-VN')} {new Date().toLocaleTimeString('vi-VN')}</div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between"><span>Mã phiếu:</span><span>{printingDeposit.deposit_number}</span></div>
                <div className="flex justify-between"><span>Khách hàng:</span><span className="font-bold text-right truncate max-w-[180px]">{customers.find(c => c.id === printingDeposit.customer_id)?.name || 'Khách vãng lai'}</span></div>
                <div className="flex justify-between"><span>Ngày tạo:</span><span>{formatDate(printingDeposit.created_at)}</span></div>
              </div>

              <div className="pt-4 border-t border-slate-200 border-dashed space-y-3">
                <div className="flex justify-between text-sm"><span>Số tiền yêu cầu:</span><span className="font-bold">{formatVND(printingDeposit.required_amount)}</span></div>
                <div className="flex justify-between text-sm"><span>Đã thanh toán:</span><span className="font-bold text-green-600">{formatVND(printingDeposit.paid_amount)}</span></div>
                <div className="flex justify-between"><span>Trạng thái:</span><span>{printingDeposit.status === 'paid' ? 'Đã thu' : printingDeposit.status === 'pending' ? 'Chờ thu' : printingDeposit.status}</span></div>
              </div>
            ...
            <div className="mt-2 mb-6 px-6 flex gap-2 w-full shrink-0 no-print">
              ...
              >
                <Printer className="w-4 h-4" /> In phiếu cọc
              </button>
              <button 
