...
export default function Appointments() {
  ...
  const updateStatus = async (appt, status) => {
    try {
      ...
      if (status === 'completed') {
        const cust = customers.find(c => c.id === appt.customer_id) || { id: appt.customer_id, name: appt.customer_name, phone: appt.customer_phone || '' };
          ...
          return (
            <div key={status}>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {list.map((a) =>
                <div key={a.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm py-4 px-3">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span className="text-[10px] font-semibold rounded-full py-1 px-5" style={{ background: STATUS_COLORS[status] + '1a', color: STATUS_COLORS[status] }}>
                          {STATUS_LABEL[status]}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
