...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Title */}
      <div>
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-800">Cài đặt hệ thống</h1>
        <p className="text-slate-400 text-sm mt-1">Cấu hình các phân hệ chi nhánh, tài khoản và quyền hoạt động.</p>
      </div>
                ...
                <div key={b.id} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col justify-between">
                  <div>
                    ...
                    <div className="space-y-1.5 mt-3">
                      ...
                      {b.manager_name && <div className="text-sm text-slate-500 flex items-center gap-1.5"><User className="w-4 h-4 text-slate-400 shrink-0" />QL: {b.manager_name}</div>}
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
            ...
            <div className="flex gap-2">
              ...
              >
                <Shield className="w-4 h-4 text-slate-500" /> Vai trò
              </button>
              <button 
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50/50 border-b border-slate-100 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                      <th className="px-6 py-4">Mã TK</th>
                      <th className="px-6 py-4">Tên hiển thị</th>
                      <th className="px-6 py-4">Số điện thoại</th>
                      <th className="px-6 py-4">Email</th>
                      <th className="px-6 py-4">Phân loại</th>
                      <th className="px-6 py-4">Vai trò</th>
                      <th className="px-6 py-4">Chi nhánh hoạt động</th>
                    ...
                    {profiles.length === 0 ? (
                      <tr>
                        <td colSpan="8" className="text-center py-10 text-slate-400">
                          Chưa có tài khoản đăng nhập nào được khởi tạo.
                        </td>
                      </tr>
                    ) : (
                      profiles.map((acc, index) => {
                        ...
                        // Type label
                        const typeText = acc.type === 'User' ? 'Khách hàng (User)' : 'Nhân viên';
        ...
        <div className="space-y-4">
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex items-start gap-3">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              **Lưu ý:** Quyền hạn đối với vai trò **Chủ Salon (Owner)** được gán mặc định tối cao và không thể thay đổi. 
              Các vai trò khác hệ thống và tự định nghĩa có thể thay đổi bật/tắt quyền xem hoặc chỉnh sửa tương ứng cho từng phân hệ.
            </p>
          </div>
            ...
            <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50/50 border-b border-slate-100 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                      <th className="px-6 py-4">Phân hệ chức năng</th>
                          ...
                          {Object.entries(group.modules).map(([moduleKey, label]) => {
                            return (
                              <tr key={moduleKey} className="hover:bg-slate-50/40 transition-colors">
                                <td className="px-6 py-3 text-slate-500 font-normal text-xs">{label}</td>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-y-auto" onClick={onClose}>
      ...
      <div className="relative bg-white w-full max-w-4xl rounded-2xl shadow-2xl transition-all flex flex-col max-h-[92vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
          <h2 className="text-xl font-bold text-slate-800">Cấu hình Chi nhánh</h2>
          ...
          {activeSubTab === 'general' ? (
            <div className="space-y-6">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-6">
                <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider text-left">Hồ sơ chi nhánh</h3>
                ...
                <div className="pb-2">
                  <ImageUpload 
                    ...
                    label="Logo chi nhánh (hiển thị trên website đặt lịch & hoá đơn giao dịch)" 
                    shape="square" 
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Tên chi nhánh <span className="text-red-500">*</span></label>
                    <input value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} placeholder="Nhập tên chi nhánh..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Quốc gia <span className="text-red-500">*</span></label>
                    <select value={f.country} onChange={(e) => setF({ ...f, country: e.target.value })} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500 bg-white">
                      ...
                      <option value="United States">Mỹ (United States)</option>
                      <option value="Singapore">Singapore</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Thành phố / Quận huyện <span className="text-red-500">*</span></label>
                    <input value={f.city} onChange={(e) => setF({ ...f, city: e.target.value })} placeholder="Nhập thành phố hoặc quận huyện..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Tỉnh / Bang <span className="text-red-500">*</span></label>
                    <input value={f.state} onChange={(e) => setF({ ...f, state: e.target.value })} placeholder="Nhập tỉnh hoặc bang..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Mã bưu điện <span className="text-red-500">*</span></label>
                    <input value={f.post_code} onChange={(e) => setF({ ...f, post_code: e.target.value })} placeholder="Ví dụ: 70000" className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Số điện thoại</label>
                    <input value={f.phone} onChange={(e) => setF({ ...f, phone: e.target.value })} placeholder="Nhập số điện thoại chi nhánh..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Địa chỉ chi tiết <span className="text-red-500">*</span></label>
                    <input value={f.address} onChange={(e) => setF({ ...f, address: e.target.value })} placeholder="Số nhà, tên đường..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Mã số thuế</label>
                    <input value={f.tax_code} onChange={(e) => setF({ ...f, tax_code: e.target.value })} placeholder="Nhập mã số thuế..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider text-left">Mạng xã hội</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Google Map</label>
                    <input value={f.google_map} onChange={(e) => setF({ ...f, google_map: e.target.value })} placeholder="Đường dẫn Google Map..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Youtube</label>
                    <input value={f.youtube} onChange={(e) => setF({ ...f, youtube: e.target.value })} placeholder="Đường dẫn kênh Youtube..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Facebook</label>
                    <input value={f.facebook} onChange={(e) => setF({ ...f, facebook: e.target.value })} placeholder="Đường dẫn trang Facebook..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Instagram</label>
                    <input value={f.instagram} onChange={(e) => setF({ ...f, instagram: e.target.value })} placeholder="Đường dẫn trang Instagram..." className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-1 focus:ring-emerald-500" />
                  </div>
                </div>
              </div>
              ...
              {/* Organization Format */}
              <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider text-left">Định dạng hiển thị</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Múi giờ <span className="text-red-500">*</span></label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Định dạng ngày</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Đơn vị tiền tệ</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Ngôn ngữ hiển thị</label>
            ...
            /* Working Time Tab */
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
              <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider text-left">Giờ hoạt động chi nhánh</h3>
              <p className="text-xs text-slate-400">Thiết lập khung giờ làm việc mở cửa và đóng cửa cho các ngày trong tuần.</p>
                ...
                {f.working_hours.map((wh, idx) => (
                  <div key={wh.day} className="flex items-center justify-between p-3 border border-slate-100 rounded-xl hover:bg-slate-50/50 transition-colors">
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      />
                      <span className="font-bold text-slate-700 text-sm w-24">{wh.day}</span>
                    </label>
                    ...
                    {wh.enabled ? (
                      ...
                      <span className="text-xs text-red-500 font-semibold px-3 py-1 bg-red-50 rounded-lg">Đóng cửa</span>
                    )}
                  </div>
                ))}
              </div>
      ...
      <div className="relative bg-white w-full md:max-w-md rounded-3xl p-6 shadow-2xl text-left flex flex-col max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between mb-4 shrink-0">
          <h2 className="text-base font-bold text-slate-800 font-body">
            {account.id ? 'Sửa thông tin tài khoản' : 'Thêm tài khoản'}
          </h2>
          <button 
            onClick={onClose} 
        ...
        <div className="space-y-3 flex-1 pb-4">
          ...
          {isFirst && (
            <div className="bg-amber-50 border border-amber-100 p-3 rounded-xl flex gap-2 text-[11px] text-amber-700">
              ...
              <p>Tài khoản đầu tiên mặc định giữ vai trò **Chủ Salon (Owner)** và có toàn quyền quản trị tối cao.</p>
            </div>
          )}

          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block font-medium text-slate-500 mb-1 text-[11px]">Tên *</label>
              <input 
                value={f.first_name} 
            ...
            <div>
              <label className="block font-medium text-slate-500 mb-1 text-[11px]">Họ & Tên đệm</label>
              <input 
                value={f.last_name} 
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block font-medium text-slate-500 mb-1 text-[11px]">Số điện thoại</label>
              <input 
                value={f.phone} 
            ...
            <div>
              <label className="block font-medium text-slate-500 mb-1 text-[11px]">Email *</label>
              <input 
                value={f.email} 
                disabled={!!account.id}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block font-medium text-slate-500 mb-1 text-[11px]">Phân loại *</label>
              <select
                value={f.type}
            ...
            <div>
              <label className="block font-medium text-slate-500 mb-1 text-[11px]">Trạng thái</label>
              <select
                value={f.status}
          ...
          <div>
            <label className="block font-medium text-slate-500 mb-1 text-[11px]">Vai trò hệ thống *</label>
            <select
              value={f.role}
              disabled={isFirst}
          ...
          {f.role !== 'owner' && (
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
              <label className="block font-medium text-slate-700 mb-2 text-xs">Chi nhánh hoạt động *</label>
              <div className="flex flex-wrap gap-3">
                {branches.length === 0 ? (
                  <span className="text-xs text-slate-400">Không tìm thấy chi nhánh</span>
                ) : (
                  branches.map(b => {
          ...
          <button 
            onClick={() => {
              if (!f.email || !f.first_name) {
                return toast.error('Vui lòng điền đầy đủ Email và Tên');
              }
              if (f.role !== 'owner' && f.branch_ids.length === 0) {
                return toast.error('Vui lòng chọn ít nhất một chi nhánh hoạt động');
              }
