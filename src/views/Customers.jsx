          ...
          <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-5">
            ...
            <div className="grid grid-cols-3 gap-2 md:gap-8 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 pl-0 md:pl-8 w-full md:w-auto text-center md:text-left">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] md:text-xs text-slate-400 mt-1">{t('customers.detail.stats_appointments', 'Lịch hẹn')}</div>
              </div>
              <div className="min-w-0">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="md:hidden flex flex-col gap-3">
                      {paginatedAppts.map((appt) => {
                        ...
                        return (
                          <div key={appt.id} className="bg-white border border-slate-100 rounded-xl p-3.5 shadow-sm space-y-3">
                            <div className="flex items-center justify-between border-b border-slate-50 pb-2">
                              <button 
                                onClick={() => setSelectedApptDetail(appt)}
                                className="font-bold text-amber-600 hover:underline text-sm"
                              >
                                #{formatAppointmentId(appt.id)}
                              </button>
                              <span className={`text-[10px] font-medium px-2 py-0.5 rounded-lg border ${st.bg}`}>
                            ...
                            <div className="flex justify-between items-center text-xs">
                              <div className="flex flex-col gap-0.5">
                                <span className="text-slate-700 font-medium">{formatDate(appt.date)}</span>
                                <span className="text-slate-400 text-[11px]">{appt.time}</span>
                              </div>
                              <div className="text-right font-bold text-slate-700 text-sm">
                                {formatVND(appt.price)}
                              </div>
                            </div>

                            <div className="space-y-2 pt-2 border-t border-slate-50">
                              <div className="flex items-start gap-2">
                                <span className="text-slate-400 text-[11px] w-[60px] shrink-0 mt-0.5">Dịch vụ:</span>
                              ...
                              <div className="flex items-start gap-2">
                                ...
                                <div className="flex items-center gap-1.5 flex-wrap flex-1">
                                  {uniqueStaff.length > 0 ? uniqueStaff.map((s, idx) => (
                                    <span key={idx} className="px-1.5 py-0.5 rounded-md bg-blue-50 border border-blue-100 text-[10px] whitespace-nowrap text-blue-600 font-normal">
                                      {s.name}
                                    </span>
                                  )) : <span className="text-slate-400 text-[10px]">Chưa phân KTV</span>}
                                </div>
                              </div>
                            </div>
                            
                            <div className="pt-2 border-t border-slate-50 flex justify-end gap-2 relative">
                              {appt.status === 'completed' ? (
                                <button 
                                  onClick={() => handleRebook(appt)} 
                                  className="px-3 py-1.5 text-[11px] rounded bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-800 transition-colors font-medium flex items-center gap-1"
                                >
                                ...
                                <div className="relative">
                                  <button 
                                    ...
                                    className="px-3 py-1.5 text-[11px] rounded bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-800 transition-colors font-medium flex items-center gap-1"
                                  >
                                    Cập nhật trạng thái <MoreHorizontal className="w-3 h-3" />
                                  </button>
                                  {activeActionId === appt.id && (
                                    <>
                                      <div className="fixed inset-0 z-40" onClick={() => setActiveActionId(null)} />
                                      <div className="absolute right-0 bottom-full mb-1 w-36 bg-white border border-slate-100 shadow-xl rounded-xl py-1 z-50 text-left text-[11px] font-normal">
                                        {appt.status === 'booked' && (
                                          <button onClick={() => handleUpdateApptStatus(appt, 'confirmed')} className="w-full px-3 py-1.5 hover:bg-slate-50 text-slate-700">Xác nhận lịch</button>
                                        )}
                                        {(appt.status === 'booked' || appt.status === 'confirmed') && (
                                          <button onClick={() => handleUpdateApptStatus(appt, 'checked_in')} className="w-full px-3 py-1.5 hover:bg-slate-50 text-slate-700">Check-in khách</button>
                                        )}
                                        {appt.status === 'checked_in' && (
                                          <button onClick={() => handleUpdateApptStatus(appt, 'completed')} className="w-full px-3 py-1.5 hover:bg-slate-50 text-green-600 font-semibold">Checkout</button>
                                        )}
                                        {(appt.status === 'booked' || appt.status === 'confirmed' || appt.status === 'checked_in') && (
                                          <>
                                            <button onClick={() => handleUpdateApptStatus(appt, 'no_show')} className="w-full px-3 py-1.5 hover:bg-slate-50 text-rose-650">Không đến</button>
                                            <button onClick={() => handleUpdateApptStatus(appt, 'cancelled')} className="w-full px-3 py-1.5 hover:bg-slate-50 text-red-500 border-t border-slate-50">Hủy lịch</button>
                                          </>
                                        )}
                                        {(appt.status === 'cancelled' || appt.status === 'no_show') && (
                                          <button onClick={() => handleRebook(appt)} className="w-full px-3 py-1.5 hover:bg-slate-50 text-primary">Rebook lịch mới</button>
                                        )}
                                      </div>
                                    </>
                                  )}
                              ...
                              <button 
                                onClick={() => handleDeleteAppt(appt.id)}
                                title="Xóa lịch hẹn"
                                className="px-2 py-1.5 rounded bg-slate-50 hover:bg-red-50 text-slate-400 hover:text-red-650 transition-colors"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
                            </div>
