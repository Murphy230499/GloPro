          ...
          <div className="space-y-2">
            {dayShifts.map((s) => (
              <div key={s.id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50">
                <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm text-white" style={{ background: staff.find(x => x.id === s.staff_id)?.avatar_color || '#FF6B9D' }}>
                  {s.staff_name?.[0]}
                </div>
          ...
          return (
            <div key={s.id} className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg text-white" style={{ background: s.avatar_color || '#FF6B9D' }}>{s.full_name[0]}</div>
...
function StaffForm({ staff, onClose, onSave }) {
  const [f, setF] = useState({
    ...
    avatar_color: staff.avatar_color || '#FF6B9D',
  });
  return (
